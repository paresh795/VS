...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const emptyRoomUrls = falResult.data.images.map((img: any) => img.url)
      const emptyRoomUrl = emptyRoomUrls[0]
      
      console.log('🖼️ [Empty Room API] Generated empty room URLs:', emptyRoomUrls)
